...
Base.@kwdef mutable struct SAC
    ...
    phs::Bool # Whether or not to impose particle-hole symettry for Hubbard like models
              # If set to true, S(-ω) = S(ω) and only the positive part of the spectrum is sampled
...
function initialize!(self)
    ...
    if self.phs   
        A_array ./= (2*sum(A_array)) # If phs, then only positive side of frequency axis is sampled,
    ...
    for i=ωi_0:ωi_m
        ω = ((i + 0.5) * self.δω)
        K_vals .= (exp.(-self.τ .* ω) ./ (exp(-self.β * ω) + 1))

        #If phs, then kernel is adjusted to account for S(-ω) = S(ω) 
        if self.phs   
            K_vals .+=  (exp.(self.τ .* ω) ./ (1 + exp(self.β * ω)))
        end
        
        # The phs part of the Kernel may contain large numbers is β is large
        # Julia treats this term as Inf/Inf = NaN,
        # To fix this, we multiply by e^{-βω}/e^{-βω}, just for the NaN values
        
        if sum(isnan.(K_vals)) != 0 
           
            if self.phs 
                K_vals[isnan.(K_vals)] .= (exp.(-self.τ[isnan.(K_vals)] .* ω) ./ (exp(-self.β * ω) + 1)
                                             .+ (exp.(-(self.β.-self.τ[isnan.(K_vals)]) .* ω) ./ (1 + exp(-self.β * ω)))) #symm
            else
                K_vals[isnan.(K_vals)] .= exp.((self.β .- self.τ[isnan.(K_vals)]) .* ω) ./ (exp(self.β * ω) + 1)
            end
...
function write_spec(self, n)
    ...
    for i=self.ωi_0:self.ωi_m
        ω_i = Integer(floor((i - self.ωi_0 + 0.5)/δω_conversion))
        if ω_i < N_h
            sampled_spec_hist[ω_i+1] += (self.norm * pi) * self.sampled_spec[i-self.ωi_0 + 1] / self.δω_h
        end
    end
    ...
    f = open(spec_outfile, "w")
        println(f, "omega,S")
        for i=1:N_h
            println(f, self.δω_h*(i-1)+(self.ωi_0)*self.δω, ",", sampled_spec_hist[i])
        end
    close(f)
...
#Method 2:
## a_v1) High initial tempearture anneal to equillibrate, from θ=100*θ_0 to θ_0 in 10 steps
##    using 1/5 annealing steps per temp (currently implemented)
## a_v2) High tempearture adjust_windows to equillibrate, at θ=10*θ_0, 5*θ_0, 2*θ_0
##    using 5 * annealing steps per temp (currently not implemented)
...
function equillibration(self)

    adjust_windows(self, self.anneal_steps * 5,  10*self.θ_0)
    adjust_windows(self, self.anneal_steps * 5,  5*self.θ_0)
    adjust_windows(self, self.anneal_steps * 5,  2*self.θ_0)
...
function run()
    ...
    output_folder = in_file[5, 1]
    phs = Bool(in_file[6, 1])
    

    # G(τ) data and covariance matrix
    G_file = "t.in"
